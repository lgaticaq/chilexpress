        ...
        if (res.statusCode !== 200) {
          ...
          res.on('end', () => {
            try {
              ...
              const resume = $('.wigdet-content > ul > li > ul > li')
                .get()
                .map(i => {
                  const h = $(i)
              ...
              if (downloadHead.length > 0) {
                const download = downloadHead
                  .parents('.wigdet-content')
                  .find('ul > li > ul > li')
                  .get()
                  .map(i => {
                    const h = $(i)
                ...
                data.delivery = new Date(
                  dst.replace(pattern, '$3-$2-$1T$4:$5:00-03:00')
                )
              ...
              data.history = tbl.map(x => {
                ...
                return {
                  datetime: new Date(
                    st.replace(pattern, '$3-$2-$1T$4:$5:00-03:00')
                  ),
                  activity: x[2]
                }
              })
